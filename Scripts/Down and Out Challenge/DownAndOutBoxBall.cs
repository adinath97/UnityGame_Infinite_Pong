...
public class DownAndOutBoxBall : MonoBehaviour
{
    ...
    void Start()
    { 
        DownAndOutLevelManager.startGame = false;
        launched = false;
        hits = 0;
    ...
    void BallMovementMetrics() {
        if(rb.velocity.y >= 0) {
            ...
            DownAndOutLevelManager.ballMovingUp = true;
        }
        else if(rb.velocity.y < 0) {
            direction = Vector3.down;
            DownAndOutLevelManager.ballMovingUp = false;
        }
    ...
    void InitialLaunchMetrics() {
        ...
        if(DownAndOutLevelManager.startGame && !launched) {
    ...
    void OnCollisionEnter2D(Collision2D collisionInfo)
    {
        ...
        if(!noCollisionsNow) {
            ...
            collisionInfo.gameObject.tag == "playerRightPart") && hits > 1) {
                ...
                DownAndOutAIPaddle.pickChasingSpot = true;
                Debug.Log(rb.velocity.magnitude);
            }
            if(collisionInfo.gameObject.tag == "playerLeftPart") {
